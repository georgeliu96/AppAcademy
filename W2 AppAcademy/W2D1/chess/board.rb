...
class Board 
    ...
    def dup 
        colorhash = {white: 1, black: 2}
        dupped = Board.new 
        dupped.grid.each_with_index do|sub, i1| 
            sub.each_with_index do |el, i2| 
                old_piece = self[[i1,i2]]
                dupped[[i1,i2]] = old_piece.class.new([[i1,i2]],colorhash[old_piece.color])
             end 
        end 
        return dupped 
    end 
