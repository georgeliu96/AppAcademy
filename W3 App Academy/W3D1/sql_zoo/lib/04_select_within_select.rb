...
def population_constraint
  ...
  # added `+1` and `-1` to get rid of inclusiveness of BETWEEN :)
end
...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
    continent NOT IN(SELECT continent FROM countries 
    WHERE population >= 25000000)
    SQL
  end
  
  # continent NOT IN (SELECT continent FROM countries WHERE name IN (
  #   SELECT name FROM countries WHERE population >= 25000000 
  # ) ) This also works but is a lot more complicated 
